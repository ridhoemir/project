...
if(isset($_POST['submit'])){
 ...
 $ver_code = md5(uniqid(rand()));
		$cek = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM penyedia WHERE email='$email'"));
		$cek2 = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM penyedia WHERE username_operator='$username'"));
		$cek3 = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM customer WHERE email='$email'"));
		$cek4 = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM customer WHERE username_vust ='$username'"));
			if ($cek > 0 || $cek3 > 0){
		echo "<script>window.alert('Email yang anda masukan sudah ada')
		window.location='user_daftar.php'</script>";
		...
		window.location='user_daftar.php'</script>";
			 ...
			 $sql = "insert into customer values ('$username','$nama','$jk','$email','$password')";
        ...
        if($query)
            {
                ...
                echo "<script> alert(\"Silakan Tunggu Dan Cek Email Anda....\"); window.location = \"user_daftar.php\"; </script>";
           		 }
            	else
            	{
                echo "<script> alert(\"Maaf!! Silakan Cek Kembali Form Yang Telah Anda Isi\"); window.location = \"user_daftar.php\"; </script>";
            	}
		}
}
  ?>
    ...
    <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
      <div class="form-group">
        ...
        <div class="col-sm-3"></div>
        <div class="col-sm-8">
          <h3><b>Sekarang Anda Mendaftar Sebagai Customer</b></h3>
        </div>
      </div>
